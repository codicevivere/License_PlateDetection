...
class Eval():
    ...
    def test_using_model_rbf(self, feature_test):   
        ...
        ##########new############
        # file_name = "ob1.pickle"    
        # classifier = pickle.load(open("ob1.pickle", 'rb'))
        # test_classify = classifier.predict(feature_test)
        # dict_models[file_name] = test_classify
        ##########new############
    ...
    def test_set_on_specific_rbf_model(self, feature_test,model):     
        modelpath=os.path.abspath(os.path.join(self.conf['ModelDir'],model ))
        classifier = pickle.load(open(modelpath, 'rb')) 
        test_classify = classifier.predict(feature_test)
        return test_classify,modelpath
